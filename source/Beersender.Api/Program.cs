...
builder.Services.AddTransient<Beer_package_router<Create_package>>(services =>
{
    ...
    return new Beer_package_router<Create_package>(id => store?.Get_events(id) ?? Enumerable.Empty<IEvent>(), @event => store?.Publish(@event));
});

builder.Services.AddTransient<Beer_package_router<Add_shipping_label>>(services =>
{
    var store = services.GetService<Sql_event_store>();
    return new Beer_package_router<Add_shipping_label>(id => store?.Get_events(id) ?? Enumerable.Empty<IEvent>(), @event => store?.Publish(@event));
});

builder.Services.AddTransient<Beer_package_router<Send_package>>(services =>
{
    var store = services.GetService<Sql_event_store>();
    return new Beer_package_router<Send_package>(id => store?.Get_events(id) ?? Enumerable.Empty<IEvent>(), @event => store?.Publish(@event));
});
